...
const (
	revenueSourceTTM4Q             = ttmSourceTTM4Q
	revenueSourceTTMPriorBridge    = ttmSourceTTMPriorBridge
	revenueSourceAnnualFY          = ttmSourceAnnualFY
	revenueSourceAnnualizedQuarter = ttmSourceAnnualizedQuarter
	revenueSourceInsufficient      = ttmSourceInsufficient
)

// revenueTTMSpec parameterizes the shared trailing-twelve-months chain
// (trailing_ttm.go) for the revenue metric. SR-1 A6 unified the previously
// duplicated revenue/operating-income chains; the fallback order, source
// identifiers and warning strings are byte-identical to the pre-unification
// behavior (pinned by trailing_revenue_test.go with assertions unchanged).
var revenueTTMSpec = ttmSpec{
	metric:           metricRevenue,
	labelPrefix:      "revenue_base",
	insufficientNoun: "revenue",
	annualizedNoun:   "single-quarter revenue",
}

// TrailingTwelveMonthsRevenue returns a best-effort trailing-twelve-months
// (TTM) revenue figure for use by EV/Revenue-style models. It walks the
// documented fallback chain (see trailing_ttm.go for the chain contract):
//
//	TTM_PRIOR_BRIDGE → TTM_4Q → ANNUAL_FY → ANNUALIZED_QUARTER → INSUFFICIENT_HISTORY
//
// The bridge runs FIRST so partial-year IPO shapes surface as
// TTM_PRIOR_BRIDGE instead of being silently absorbed into TTM_4Q (V/R/Q
// follow-up 9da6c68). The signature is stable: replay tooling
// pattern-matches `source` strings (RM-1 spec) — adding a source requires a
// spec update; renaming one is a breaking change.
//
...
// and break replay determinism (RM-1.A Option B — clock seam is
// ModelInput.Now plumbed from *Service.clock).
func (h *HistoricalFinancialData) TrailingTwelveMonthsRevenue() (revenue float64, source string, warning string) {
	return h.trailingTwelveMonths(revenueTTMSpec)
}

// ttmPriorBridgeRevenue delegates to the shared bridge tier (SR-1 A6). Kept
// as a named helper because unit tests exercise the bridge in isolation.
func (h *HistoricalFinancialData) ttmPriorBridgeRevenue() (float64, bool) {
	return h.ttmPriorBridgeOf(metricRevenue)
}

// annualizedQuarterRevenue delegates to the shared annualized tier (SR-1 A6).
// Kept as a named helper because unit tests exercise the tier in isolation.
func (h *HistoricalFinancialData) annualizedQuarterRevenue() (float64, bool) {
	return h.annualizedQuarterOf(metricRevenue)
}
