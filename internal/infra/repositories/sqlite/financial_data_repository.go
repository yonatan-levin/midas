...
func (r *FinancialDataRepository) StoreHistorical(ctx context.Context, data *entities.HistoricalFinancialData) error {
	...
	// Store each period individually - SQLite handles ACID properties well
	// TODO: Implement proper transaction handling with interface segregation
	for _, periodData := range data.Data {
		err := r.Store(ctx, periodData)
		if err != nil {
