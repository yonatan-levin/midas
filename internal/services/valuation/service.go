...
func (s *Service) performValuation(
	...
	result := &entities.ValuationResult{
		...
		CalculationVersion:    "4.6", // BUG-015: standard-DCF operating-income base annualized to TTM for 10-Q-latest tickers (was a single quarter, ~4× understated). Changes dcf_value_per_share for quarter-latest firms by design; FY-latest bit-for-bit unchanged. Prior: 4.5 (BUG-014 DCF operating NWC excludes cash).
...
func (s *Service) performAlternativeValuation(
	...
	result := &entities.ValuationResult{
		...
		CalculationVersion:    "4.6", // BUG-015: standard-DCF operating-income base annualized to TTM for 10-Q-latest tickers. Alt-model numerics (DDM/FFO/revenue_multiple) are unaffected by the OI-base change (those paths do not use the standard-DCF OI base) — only calculation_version drifts here (bit-for-bit primary values). The bump is engine-wide for a single version stamp. Prior: 4.5 (BUG-014).
