...
func TestIndustryCodeDetectorIntegration(t *testing.T) {
	...
	t.Run("DetectByKeywords", func(t *testing.T) {
		input := ports.IndustryDetectionInput{
			...
			Description: "We provide cloud-based software solutions for businesses",
		}
	...
	t.Run("GetIndustryByCode", func(t *testing.T) {
		...
		assert.Len(t, info.SubIndustries, 2)
...
func TestIndustryConfigLoading(t *testing.T) {
	...
	t.Run("LoadFromEnvironment", func(t *testing.T) {
		...
		os.Setenv("INDUSTRY_CODE_CONFIG_PATH", configPath)
		defer os.Unsetenv("INDUSTRY_CODE_CONFIG_PATH")
		...
		"mappings": [
			{
				...
				"matchers": {
					...
					"exact_names": ["Microsoft Corporation"]
				},
				"sub_industries": [
					{
