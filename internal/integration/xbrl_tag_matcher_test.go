...
func TestXBRLTagMatcherIntegration(t *testing.T) {
	...
	t.Run("MatchTags_AlternativeTags_Success", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.GreaterOrEqual(t, len(results), 4, "Should match alternative tags")
...
func TestXBRLConfigurationLoading(t *testing.T) {
	...
	t.Run("LoadFromEnvironmentVariable", func(t *testing.T) {
		...
		os.Setenv("XBRL_CONFIG_PATH", configPath)
		defer os.Unsetenv("XBRL_CONFIG_PATH")
	...
	return `{
		...
		"tag_mappings": {
			"assets": {
				...
				"data_type": "decimal",
				"required": true,
				"transformations": ["to_decimal", "multiply_by_thousand"],
