...
- Tickers whose SEC response has no usable US-GAAP XBRL split into two distinct error codes (Phase A of the IFRS-FPI plan, `docs/refactoring/ifrs-foreign-private-issuer-support-spec.md`):
  1. **Foreign private issuers** filing 20-F with `ifrs-full` taxonomy (TSM, ASML, NVO, AZN, SAP, BABA, …) → `HTTP 422 FOREIGN_PRIVATE_ISSUER_UNSUPPORTED`. Wraps `ports.ErrForeignPrivateIssuer` → `valuation.ErrForeignPrivateIssuer`. Classification happens in `sec/parser.go:classifyEmptyParseError` based on which taxonomies the response carried; service-layer propagation goes through `hasForeignPrivateIssuerError`. Phase B will teach the parser to read IFRS so this case stops firing.
  2. **Clinical-stage biotechs / pre-revenue companies** with `us-gaap` present but no Revenue/OperatingIncome → `HTTP 422 INSUFFICIENT_DATA`. Wraps `ports.ErrCompanyFactsNotFound` → `valuation.ErrInsufficientData`. Same path as a true SEC 404 from `sec/client.go` (genuinely unknown CIK). Service-layer check is `hasCompanyFactsNotFoundError`.

  In both branches the handler **must check FPI first** because it is the more specific case; falling through to the generic INSUFFICIENT_DATA branch would mask the actionable diagnostic.
