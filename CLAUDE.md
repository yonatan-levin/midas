...
| `internal/services/valuation/profile/` | **Tier 2 AssumptionProfile package** (Bootstrap stub at `profile.go`; P0a replaces with full type system: `AssumptionProfile` struct + Archetype/Maturity enums + `Facts` DTO + `Registry` interface + `Resolve` 3-stage algorithm + `ResolutionTrace`). Profile resolution fires in `service.go::performValuation` before `router.SelectModel`; resolved profile stamped onto `ModelInput.Profile`. |
