...
- [ ] **Task 2.5.3: Contract Fuzzing (1 hour)**  
  **Sub-tasks**:  
    - [X] Feed generated OpenAPI spec to Schemathesis.  
    - [X] Auto‑fuzz valid/invalid inputs.  
    - [X] CI fails on unexpected 5xx or schema drift.  
...
- [ ] **Task 2.5.4: Smoke CLI & Seed (1 hour)**  
  **Sub-tasks**:  
    - [X] `scripts/smoke.sh` curls AAPL value.  
    - [X] SQL seed inserts demo API key.  
...
- [ ] **Task 2.5.5: Perf Baseline (2 hours)**  
  **Sub-tasks**:  
    - [X] Run k6 20 RPS for 60 s.  
    - [X] Capture p95 latency.  
    - [X] pprof hotspot inspection.  
    - [X] Optimize JSON parsing or cache per findings.  
...
- [ ] **Task 2.5.6: MVP Freeze Checklist (1 hour)**  
  **Sub-tasks**:  
    - [X] Create `docs/MVP_CHECKLIST.md`.  
    - [X] Tag `v0.9.0-rc1`.  
    - [X] Only bug‑fix PRs allowed until checklist green.  
...
- [X] **Task 3.1: Implement AI Footnote Parsing (6-8 hours)**  
  **Sub-tasks**:  
    - [X] TDD: Mock AI service tests.  
    - [X] services/datacleaner/ai/: Real implementation using external API.  
    - [X] Integrate into B3 contingent liabilities.  
  **Reasoning**: (1) Matches TODOs; (2) Enhances accuracy; (3) Future-proofs per docs. Best as it's modular and tested.

- [X] **Task 3.2: Add Nightly Ingestion Scheduler (4 hours)**  
  **Sub-tasks**:  
    - [X] TDD: Tests for scheduler.  
    - [X] services/datafetcher/: Cron job for bulk SEC updates.  
  **Reasoning**: (1) Per rules roadmap; (2) Keeps data fresh; (3) Automates per generalspecdoc.mdc. Best for reliability.

- [X] **Task 3.3: Swagger Docs & Final Polish (6 hours)**  
  **Sub-tasks**:  
...
⁷ **Bulk E2E Test Added (2025-08-11)**: Implemented `TestE2E_FairValue_BulkRequest` in `internal/integration/service_test.go`. Uses full DI server with mock SEC for determinism; passes on Windows. Live E2E tests remain gated by `E2E_LIVE=1` and are skipped on Windows.

[⁸] Phase 2.5 Steps 4–5: Integration coverage at 95.9% (Windows-safe), added invalid input contract fuzz (no 5xx), OS-specific Redis fallback to memory in tests to ensure deterministic local runs. Full `go test ./...` green.
