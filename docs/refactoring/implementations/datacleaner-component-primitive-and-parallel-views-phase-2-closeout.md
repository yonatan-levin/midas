...
| `TestDataCleanerRecompute_ShadowMode_TickerBasket` shadow snapshots | `internal/integration/testdata/recompute-shadow/<TICKER>.json` | Byte-identity preserved through PR-3 (predicted-zero per plan §4 row C — earnings adjusters touch income-statement fields invisible to the recompute math). PR-4 expected to mutate snapshots slightly (B-rules touch balance-sheet umbrellas); the new `TestLedger_BasketSnapshot_ClusterPrediction` (PR-4 Task 4.6) is the structural-regression sentinel that catches "an adjuster was dropped" without relying on snapshot byte-identity. |
