...
- **Files:**
  ...
  - Create: `docs/refactoring/archive/datacleaner-component-primitive-and-parallel-views-phase-2-closeout.md` (template from Phase 1 closeout).
...
Create: `docs/refactoring/archive/datacleaner-component-primitive-and-parallel-views-phase-2-closeout.md` using the Phase 1 closeout as the template. Mandatory sections:
- What landed (per PR)
...
| 2026-05-23 | PR-4 SHIPPED on branch `dc1-phase-2-pr-4` — Phase 2 COMPLETE. 7 commits: 4.1 B1 operating_lease_capitalization OverlayEmitter (`Field:"TotalDebt"`) `613773a`; 4.2 B2 pension_underfunding OverlayEmitter (`Field:"TotalDebt"`) `e328226`; 4.3 B3 contingent_liability OverlayEmitter (`Field:"DebtLikeClaims"` — Phase 4 routing intent; today's Phase 2 dual-write still mutates `data.TotalDebt`) with best-effort `AIProvenance` (empty hashes per Q4 deferral) `5c4e88d`; 4.4 orchestrator `data.TotalDebt += result.Amount` absorbed into `ProcessLiabilityAdjustments`' per-rule switch arms (Option α) `37f7400`; 4.5 liability-side shim branch deleted + both shim helpers (`shimLedgerEntriesFromLegacy` + `shimLedgerEntriesFromLegacyExcluding`) removed (PR-1 shim FULLY gone) `5eaadd3`; 4.6 `TestLedger_BasketSnapshot_ClusterPrediction` integration test at `internal/integration/datacleaner_ledger_basket_test.go` (10/10 basket tickers PASS) `f2c5d0e`; 4.7 Phase 2 closeout docs sweep — CLAUDE.md per-PR sub-bullets consolidated + spec/plan changelogs + DC-1 tracker progress + T2-BS-3 (Option B carve-out) + THESIS/AGENTS/ARCHITECTURE/TESTING + NEW Phase 2 closeout doc at `docs/refactoring/archive/datacleaner-component-primitive-and-parallel-views-phase-2-closeout.md` (commit SHA to be inserted after Task 4.7 lands). All 17 cleaner-side adjusters now native (6 A-rules + 7 C-rules + 3 B-rules) + PR-1 shim FULLY deleted across all 3 categories. Four role flavors locked: OverlayEmitter, Restater, Restater+TaxShieldDTA, FlagEmitter. SchemaVersion stays at 8. T2-BS-3 Option B carve-out chosen. All load-bearing invariants GREEN throughout: `TestDDM_LegacyPath_BitForBit` (non-trivial in PR-4 because B-rules touch `TotalDebt`), `TestRecomputeUmbrellas_NoMutation`, `TestOrchestrator_LedgerOrdering`, `TestDataCleanerRecompute_ShadowMode_TickerBasket` + shadow-snapshot byte-identity, and the new `TestLedger_BasketSnapshot_ClusterPrediction`. PR-4 final REVIEWER pass is the last gate before HUMAN signoff + full 4-PR-stack merge to master. Phase 3 (view reconstruction) is the next architectural gate. |
