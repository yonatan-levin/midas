...
9. **`internal/services/datacleaner/service.go`** — focus on `Clean(ctx, ...)` (Phase 3 adds `CleanFinancialDataWithViews(ctx, ...)` sibling) and `applyActiveAdjustments(ctx, ...)` (Phase 3 updates the call signatures it dispatches to).
...
| 3.5 | Add `Service.CleanFinancialDataWithViews(ctx, ...)` sibling to `service.go` | `internal/services/datacleaner/service.go` (add ~10 LOC method; no existing method changes) | `TestCleanWithViews_ReturnsWrapper` GREEN |
...
| `CleanedFinancialData` is ADDITIVE | Existing `data.*` reads continue to work until Phase 4 migration. **Phase 3 adds `CleanFinancialDataWithViews(ctx, ...)` sibling, does NOT change `Clean(ctx, ...)` signature.** | `service.go` audit |
...
| 2026-05-24 | Phase 3 SHIPPED on branch `dc1-phase-3`. All 14 tasks completed in single-PR Option A across 8 implementation commits: `014b310` (3.1-3.4 + 3.11 — cleaneddata package skeleton + AsReported / Restated / InvestedCapital accessors + import-boundary test); `7855027` (3.5 — Service.CleanFinancialDataWithViews sibling + MockDataCleanerService mock alignment); `54c14c6` (3.6 — T2-BS-3 carve-out test in cleaneddata package); `708e5ce` (3.9 — ctx threading through all three Process*Adjustments + test-callsite + 5 shadow-fix files via `gocontext "context"` alias); `146c42b` (3.7 + 3.10 — Q2 A2 TaxShieldDTA population + SchemaVersion 8→9 atomic); `4f634a7` (3.8 — Q4 B3 AIProvenance SHA-256 hashes); `deac62b` (3.12 — live AMD/KO Restated reconstruction integration test); `f2fba9a` (3.10 follow-up — bump replay happy-bundle testdata 8→9). Closeout doc filed at `datacleaner-component-primitive-and-parallel-views-phase-3-closeout.md` (commit `c9553fd` — Tasks 3.13 + 3.14). |
| 2026-05-25 | V/R/Q-driven cleanup commit `b997ce6` landed on `dc1-phase-3` (10th commit on the branch). Addresses REVIEWER Tier-2 (explicit `default:` clauses in `cleaneddata/restate.go` + `cleaneddata/invested_capital.go` switches; stale "Phase 3's planned ctx threading" narrative rewritten in `liabilities.go::captureB3AIProvenance` godoc to describe post-Phase-3 reality) and VERIFIER docs-nit (spec + plan + handoff aligned to the production method name `CleanFinancialDataWithViews`, preserving the actual test name `TestCleanWithViews_ReturnsWrapper`; closeout commit-count framing clarified). No code semantics change; all load-bearing invariants stayed GREEN; full `go test ./...` 0 failures. |
