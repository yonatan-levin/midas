...
| 2026-05-25 | **Phase 3 followup SHIPPED** on branch `dc1-phase-3-followup` (10 commits — F.1 through F.10 per the implementer plan). All 9 acceptance gates GREEN. New load-bearing pin `TestCleanFinancialDataWithViews_Restated_NoDoubleCount_OnRestaterFire` added. NON-goals preserved (no consumer migration, no B3 routing flip, no dispatcher dual-write deletion, no `CalculationVersion` bump, no `SchemaVersion` bump, no changes to `recompute.go`, no DDM golden regeneration). Closeout: `docs/refactoring/implementations/dc1-phase-3-followup-closeout.md`. |
