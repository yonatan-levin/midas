...
**Open question CLOSED here**: should the new function be additive (`CalculateEquityValueWithDebtLikeClaims`) or replace `CalculateEquityValue` in-place? Answer: ADDITIVE. The legacy 5-arg signature stays for backwards compatibility with tests + the alt-model `revenue_multiple`/`ffo` paths which build equity differently (they compute `equityValue = enterpriseValue - input.InterestBearingDebt + input.CashAndCashEquivalents` inline; those sites flip to read `restated.InterestBearingDebt`).

> **CORRECTION (commit `2ea9978`, gpt-5.5 cross-model review — see closeout §7.6):** the original sentence above continued "…but do not gain the `DebtLikeClaims` subtraction in Phase 4 — alt models … don't have a B3 contingent-liability story today." That was correct for **`ffo`** but **WRONG for `revenue_multiple`**. `revenue_multiple` computes a genuine enterprise value (`EV = revenue × EV/Revenue multiple`) and bridges `EV − InterestBearingDebt + Cash`; after Phase 4's dual-write deletion `restated.InterestBearingDebt` is B-rule-free, so omitting `DebtLikeClaims` **silently dropped** the B1 (lease) / B2 / B3 claims that were previously folded into `InterestBearingDebt`, overstating equity. `revenue_multiple` therefore DOES subtract `DebtLikeClaims` (via `ModelInput.DebtLikeClaims`). `ffo` does NOT — its equity is multiple-derived (`valuePerShare × shares`) and `InterestBearingDebt` only back-derives the reported EV, so subtracting there would double-count.
...
- For **revenue_multiple**: `modelInput.InterestBearingDebt = restated.InterestBearingDebt` AND `modelInput.DebtLikeClaims = investedCapital.DebtLikeClaims`; the EV→Equity bridge subtracts both — `EV − InterestBearingDebt + Cash − DebtLikeClaims`. **CORRECTION (commit `2ea9978`, post-gpt-5.5-review — supersedes the original "alt models do NOT subtract DebtLikeClaims" text):** `revenue_multiple` computes a genuine enterprise value (EV/Revenue multiple), so after the Phase 4 dual-write deletion made `restated.InterestBearingDebt` B-rule-free, the bridge MUST re-subtract `DebtLikeClaims` or it silently drops B1/B2/B3 (esp. B1 lease) claims and overstates equity. Same accuracy-correction class as the DCF B3 routing flip (§5.3).
- For **ffo**: `modelInput.InterestBearingDebt = restated.InterestBearingDebt`; does NOT subtract `DebtLikeClaims` — FFO equity is multiple-derived (`valuePerShare × shares`) and `InterestBearingDebt` only back-derives the reported EV, so subtracting `DebtLikeClaims` would double-count. (`ModelInput.DebtLikeClaims` is populated for FFO too but left unread.)
- For **DDM**: `modelInput.InterestBearingDebt = latestFinancialData.InterestBearingDebt`, `modelInput.DebtLikeClaims = 0` (UNCHANGED — preserves bit-for-bit). See §9.
