...
**Version:** 0.5
**Date:** 2026-05-09
**Status:** **ALL R0–R3 SHIPPED on master. Phase 2.D COMPLETE.** R0+R1 merge `8a9878f` (2026-05-03); R2 merge `e4d2fb2` (2026-05-05); R3a merge `011d78c` (2026-05-06); R3b merge `0741958` (2026-05-09 — 11 commits across 1 BACKEND dispatch + 3 polish commits + V/R/Q gate cycle returning zero MAJOR/BLOCKER findings). R3b closes the 5 R3a-deferred stages: Stage K (`--diff-stages` engine wiring + `stage_diff.go`), Stage L.1 (verbose stage-diff text render), Stage M.3 (parsed-mode round-trip via `seedFullBundle_ParsedMode`), Stage M.1 (6 JSON contract golden fixtures + `UPDATE_GOLDEN=1` regeneration harness), Stage N (perf benches NF2/NF3 with synthetic 100-bundle corpus generator inlined into `replay_bench_test.go`, bench-gated via TestMain), Stage O.6 (`init()` reflection guard for `countFairValueFields` against `FairValueResponse + Industry + SanityCheck = 23 + 5 + 8 = 36`). All 16 RPL-3 follow-ups closed (8 LOW NITs swept across `257ff5c` + `a5f08f3` + `b7a9bdc`; missing `evaluateBundleWithRecover` panic-coverage test landed; R2 modernization sweep folded in). V/R/Q polish commit `573e517` folded REVIEWER #2-#4 (thread-safety doc on `evaluateBundleFn` seam, empty-marker doc on `Result.StageDiffs`, `StageDiff.Empty()` helper) + QA M3/D1/B2 (`.gitattributes` LF-pin for golden fixtures, `--from=raw` error annotated with `--from=parsed` hint, empty bundle directory warns + exits 2). 4 deferred items tracked at `docs/reviewer/RPL4-r3b-followups.md` (1 spec/sample documentation call + 2 cross-platform polish + 1 documented coverage residual per §6 escape clause).
...
# Note: response-level diffs render BEFORE the "Stage diffs:" section without
# their own header (this is the as-shipped layout in v0.5; v0.4's sample showed
# the inverse order with explicit "Response diffs:" header — flipped during
# the R3b post-shipment docs dispatch to match implementation, closing RPL-4a).
# Format string is `(rel_drift=%.6f)` rather than `+X.XX%` per the same
# documented Deviation #3 on the merge-commit body of `0741958`.
```
...
| 2026-05-05 | v0.3 — R0 + R1 + R2 SHIPPED on master. **R0+R1 merge `8a9878f`** (2026-05-03, 8 commits): `valuation.Clock` injection (D10), replay skeleton with manifest validation, schema-drift detection, bundle walk with symlink follow-once, float-tolerant diff helpers, `ErrBundleMissingPayload` sentinel, text + JSON renderers. **R2 merge `e4d2fb2`** (2026-05-05, 17 commits across 3 BACKEND review cycles + 4 review gates): bundle gateways for SEC/Market/Macro/YFinance, side-effect stubs, `replay.Module` fx composition, `Replay()` orchestrator, `--from=raw\|parsed` CLI flag, headline round-trip + cross-year integration tests, `go-cmp` direct-import. **Stage C deviation (documented post-shipment)**: spec §5 D2 said `fx.Decorate` over `CoreModule + ServiceModule`; implementation hand-picks individual `fx.Provide` lines because CoreModule transitively pulls `*sqlx.DB` and `*redis.Client` constructors that would side-effect even when downstream consumers are decorated away. The hand-pick strengthens F11 hermeticity at the cost of needing module updates when production engine constructor signatures change. Documented in 60-line package comment at `internal/observability/replay/module.go:23-60`. **Coverage**: replay 84.5%, cmd/replay 81.4%, valuation 89.1% (improved from 88.7% baseline). `pkg/finance/*` byte-for-byte unchanged; `go.mod` adds only `github.com/google/go-cmp v0.7.0` direct-promotion (transitive→direct via testify). **R3 deferred** — the 15 advisory items surfaced by R2's REVIEWER + QA gates are tracked at `docs/reviewer/archive/RPL2-r2-followups.md`; R3 covers parallel batch (`--workers`), filter flags (`--filter-ticker`/`--filter-since`), `--diff-stages`, `--verbose`, JSON contract golden tests, and perf benches (NF2/NF3). Implementation plan for R2 (now historical): `docs/refactoring/observability-replay-tooling-r2-implementation-plan.md` v2. |
| 2026-05-06 | v0.4 — **R3a SHIPPED on master via merge `011d78c`** (11 commits across 4 BACKEND continuation cycles + 4 review-gate cycles). R3a covers 9 of 14 R3 plan stages: Pre-Flight parallel-fx.App lifecycle spike PASSES under `-race -count=10`; Stage I.0 Prometheus DefaultRegisterer audit + permanent CI lint scripts (`scripts/lint-prometheus-registers.{sh,ps1}`); Stage I.1 + I.5 walk thread-safety (sequential walk + per-bundle parallel replay per Decision I.1.b); Stage I.2 hand-coded bounded worker pool (`--workers` flag, NaN/-1 rejection at parse); Stage J `--filter-ticker` (case-sensitive) + `--filter-since` (consumes `ParseDurationExtended` from R0+R1); Stage L.2 `--float-rel-tol` + `--float-abs-tol` (NaN + ±Inf + negatives all rejected at parse); Stage L.3 walk vs replay timing instrumentation (`Summary.WalkDurationMs` + `Summary.ReplayDurationMs` snake_case JSON); Stage M.2 honest round-trip rename (`TestRoundTrip_ReplaySelfConsistency_ZeroDiffs` with documented self-referential limitation); Stage O sweep (9 of 12 RPL-2 cleanup items) + Stage O.13 `cmd/server` ↔ `replay` import-boundary CI guard (`os/exec` + `go list -deps`, no new modules). **Architectural deviations (documented in plan v2 + this changelog)**: (a) Stage I.2 RPL-2g half-fix turned out doc-only — I.0 audit confirmed `metrics.NewService` already allocates a fresh per-instance `*prometheus.Registry` not the global `DefaultRegisterer`, so the no-op stub the plan prescribed was redundant; per-instance isolation now CI-enforced by lint script. (b) Worker pool is hand-coded bounded semaphore rather than `errgroup.SetLimit` — avoids `golang.org/x/sync` direct-promotion (NF1 invariant). (c) F11 hermeticity preserved under parallelism via `evaluateBundleWithRecover` outer wrapper + per-app `*prometheus.Registry` isolation (verified by Pre-Flight spike). **Coverage**: replay 84.4% (defensive paths; gap concentrated in deferred Stage K/M.1 surfaces), cmd/replay 87.2% (up from 76.1% R2 baseline), valuation 89.1% no regression. `pkg/finance/*` byte-for-byte unchanged; `go.mod`/`go.sum` unchanged (no new external Go modules). **R3b deferred** — 5 stages (K `--diff-stages`, L.1 verbose stage-diff, M.1 JSON golden tests, M.3 parsed-mode round-trip, N perf benches NF2/NF3, O.6 `init()` reflection guard) + 8 LOW NITs (forvar/rangeint/stringscutprefix/etc) + missing `evaluateBundleWithRecover` panic-coverage test — tracked at `docs/reviewer/archive/RPL3-r3a-followups.md`. R3b is one well-scoped future BACKEND dispatch that completes Phase 2.D. Implementation plan for R3 (now historical, R3a stages SHIPPED): `docs/refactoring/observability-replay-tooling-r3-implementation-plan.md` v2. |
| 2026-05-09 | v0.5 — **R3b SHIPPED on master via merge `0741958`** (11 commits: 7 stage commits + 1 outcome-table commit + 3 polish commits — 1 BACKEND dispatch + 2 iterative-cleanup sweeps + 1 V/R/Q polish). **Phase 2.D COMPLETE.** All R0–R3 SHIPPED across 4 merges from 2026-05-03 (`8a9878f`) through 2026-05-09 (`0741958`). R3b closes the 5 R3a-deferred stages: Stage K (`905b295`) wires `--diff-stages` end-to-end with new `internal/observability/replay/stage_diff.go` (`StageDiffInventory`, `StageDiff` type with `Floats`/`Strings`/`DriftedWithinTolerance`+`HasMismatch()`+`Empty()`, `diffStage` core walker reading bundle JSON via `os.ReadFile`); Stage L.1 (`b87b3b7`) extends `--verbose` text render with sorted-by-stage-filename per-stage diff sections; Stage M.3 (`145b23d`) closes the parsed-mode round-trip integration gap (`seedFullBundle_ParsedMode` helper alongside the existing raw-mode); Stage M.1 (`339a273`) ships 6 JSON contract golden fixtures + `UPDATE_GOLDEN=1` regeneration harness pinning the `RenderJSON` output shape (including Stage K's new `stage_diffs` field); Stage N (`ab4b02b`) introduces NF2 (≤200ms single-bundle, 3× CI slack) + NF3-Sequential and NF3-Parallel (≤30s 100-bundle batch, 3× CI slack) benches with synthetic 100-bundle corpus generator inlined into `replay_bench_test.go` and `TestMain`-bench-gated to avoid generating on default `go test ./...`; Stage O.6 (`a990173`) adds `init()` reflection guard at `diff.go:36` that panics at package load if `countFairValueFields() = 36` disagrees with `reflect.TypeFor[handlers.FairValueResponse]().NumField() + reflect.TypeFor[handlers.Industry]().NumField() + reflect.TypeFor[entities.SanityCheck]().NumField() = 23 + 5 + 8`, with panic scope confined to the replay binary per R3a Stage O.13's `cmd/server/import_boundary_test.go` CI guard. R3b-Final cleanup commit (`257ff5c`) sweeps 8 LOW NITs (RPL-3g/h/i/j/l/m/n/p — `forvar`/`rangeint`/`stringscutprefix`/`mapsloop`/`interface{}→any`) + missing `evaluateBundleWithRecover` panic-coverage test (RPL-3o, using a 1-LoC `evaluateBundleFn` package-var seam restored via `t.Cleanup`) + R2-era modernization. Two iterative-cleanup commits (`a5f08f3` post-impl NIT sweep on R3b's own new code, `b7a9bdc` final NIT sweep with explicit cutoff line drawn against gopls's iterative-diagnostic surfacing). **V/R/Q gate cycle (zero MAJOR/BLOCKER)**: VERIFIER VERIFIED WITH NOTES (Stage O.6 panic-injection smoke CONFIRMED — temporary `VerifierJunkField` triggered drift panic with exact field-count breakdown message, reverted cleanly), REVIEWER APPROVED WITH NOTES (Stage K K.1 fallback judged "better than the plan's documented fallback"), QA PASS WITH NOTES (G1-G7 acceptance all PASS; real-bundle smoke against MXL produced operator-actionable verbose stage-diff output). **Pre-merge V/R/Q polish commit (`573e517`)** folded REVIEWER #2-#4 (thread-safety doc on `evaluateBundleFn` seam mirroring `gitSHAResolver` convention; empty-marker doc on `Result.StageDiffs` clarifying `"15-valuation.json": {}` is intentional vs absent-key; `StageDiff.Empty()` helper at `stage_diff.go:103`) + QA M3/D1/B2 (`.gitattributes` LF-pin for `internal/observability/replay/testdata/golden/*.json` preventing Windows core.autocrlf=true CI breakage; `--from=raw` error annotated with `(hint: this bundle may only have parsed snapshots; try --from=parsed)` when wrapping `ErrBundleMissingPayload`; empty bundle directory now emits stderr warning + exits 2 instead of silent SUMMARY 0/0 + exit 0). **Stage K K.1 deviation (documented)**: BACKEND used ephemeral temp-dir bundle via `os.MkdirTemp` + `Bundle.Close` synchronous drain rather than the plan's tee-writer default — REVIEWER independently judged this "better than the plan's documented fallback" (avoids the 2× per-bundle cost of re-running the engine). D7 hermeticity preserved (temp dir cleaned via `defer os.RemoveAll`; never persists). **Coverage**: replay 82.5% (gate ≥90% — documented residual per §6 escape clause; uncovered branches are defensive `if err != nil` write-error paths in `writeStageDiffSection` and `RenderJSON` plus zero-coverage `walkSlice`+`genericEqual` array-shape catch-alls with no production reach), cmd/replay 87.7% (above gate), valuation 89.6% (slight improvement from 89.1% baseline). **Cross-branch integration (zero textual conflicts)**: master diverged 10 commits in parallel during the R3b session (RM-1, RM-2, VAL-3 P1+P4, VAL-4 Graham PR #2, REIT subsector multiples, plus 5 reviewer trackers); zero file overlap with R3b's surface. Stage O.6 reflection guard verified safe pre-merge: master's FairValueResponse (23) + Industry (5) + SanityCheck (8) = 36 = `countFairValueFields()` return value. **Invariants preserved**: `pkg/finance/*` byte-for-byte unchanged (NF4); `go.mod`/`go.sum` unchanged (NF1 — `maps` package is Go 1.21+ stdlib, no module bump); F11 hermeticity (`replay.Module` adds no production-network providers); D7 (no bundles of bundles — Stage K's snapshot in-memory only); D8 (Stage K reads bundle JSON files directly via `os.ReadFile`, decoupling from entity-shape evolution). **4 deferred items tracked at `docs/reviewer/RPL4-r3b-followups.md`**: RPL-4a (Stage L.1 section-order vs spec §7 sample — REVIEWER #1, recommended resolution: accept as Deviation #8 in this changelog), RPL-4b (Windows backslash JSON path normalization — QA M2), RPL-4c (cleaner's `as_of` nondeterminism out of replay scope — QA M1), RPL-4d (documented coverage residual per §6). RPL-3 marked RESOLVED with per-RPL-3 commit-SHA mapping in the post-merge cleanup commit. Implementation plan for R3b (now historical, all stages SHIPPED per §10): `docs/refactoring/observability-replay-tooling-r3b-implementation-plan.md` v1. |
