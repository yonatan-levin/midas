...
| **5 — Cleanup: delete `Raw()` migration escape hatch** | `cleaneddata.CleanedFinancialData.Raw()` (and any matching dispatcher pointer-returns) becomes unreachable once Phase 4 closes the consumer migration. Delete it. Optional: promote the three accessor methods to `sync.Once`-protected reads if a parallel-read consumer landed in Phase 4. | 1-2 days | `grep -r '\.Raw()' internal/` returns no matches; `go test ./...` exit 0 |
