...
| **4 — Consumer migration + WACC boundary** | Single PR with 5 commit clusters: plumbing → working-capital/ROIC → DCF/cross-check → WACC/bridge/B3 routing flip → Graham/tangible/closeout. 12 of 13 consumer sites migrate; DDM deferred to Phase 5 to preserve `TestDDM_LegacyPath_BitForBit`. B3 routing flip realizes (`InvestedCapital().DebtLikeClaims` consumed in EV→Equity bridge); A1/B1/B2/B3 + A2/A4/A5/C-rule dispatcher dual-writes deleted; `CalculationVersion` 4.2 → 4.3 atomic with cluster C-4. **Spec:** [datacleaner-component-primitive-and-parallel-views-phase-4-spec.md](../archive/datacleaner-component-primitive-and-parallel-views-phase-4-spec.md). **Plan:** [datacleaner-component-primitive-and-parallel-views-phase-4-implementation-plan.md](../archive/datacleaner-component-primitive-and-parallel-views-phase-4-implementation-plan.md). | 12-18 agent-hours (single PR) | Replay basket: Class I (zero-Restater) byte-identical except CalcVersion field; Class III (B-rule firers) drift matches §5 expectation; `TestDDM_LegacyPath_BitForBit` GREEN at every commit |
| **5 — DDM migration + DebtLikeClaims correction + translator/struct retirement + cleanup** | **DDM consumer migration** (deferred from Phase 4 per Phase 4 spec §7 to preserve the `TestDDM_LegacyPath_BitForBit` cross-Tier-2 contract) — flip `ddm.go:178/181/213/291/292` reads to `Restated()` via a now-populated-for-DDM `ModelInput.LatestRestatedView`, with a 4-step bit-for-bit re-proof. **NEW DDM DebtLikeClaims correction** — DDM's EV↔equity bridge silently drops B-rule claims for B-rule-firing banks (the DDM analog of the Phase 4 `revenue_multiple` finding); thread `ModelInput.DebtLikeClaims` into the bridge (ADD, since DDM derives EV from equity). **Translator/struct retirement** — migrate the cleaner orchestrator's flag/adjustment aggregation onto the native `AdjustmentLedger`/`Overlays`/`Flags`, THEN delete the per-rule `*AdjusterOutputToLegacyResult` translators + adjustments-package `{Asset,Liability,Earnings}AdjustmentResult` structs + dead `entities.{Asset,Liability}AdjustmentResult` duplicates + dormant `earnings.go` legacy-fallback umbrella mutations. **Cleanup** — delete `cleaneddata.CleanedFinancialData.Raw()` (TODO(phase-5) marker); formalize the accessor request-local contract (no `sync.Once` — recommended); optionally stop populating the legacy `historicalData` slot (verify-then-decide). `CalculationVersion` 4.3 → 4.4 atomic with the DDM DebtLikeClaims correction. **Spec:** [datacleaner-component-primitive-and-parallel-views-phase-5-spec.md](../archive/datacleaner-component-primitive-and-parallel-views-phase-5-spec.md). **Plan:** [datacleaner-component-primitive-and-parallel-views-phase-5-implementation-plan.md](../archive/datacleaner-component-primitive-and-parallel-views-phase-5-implementation-plan.md). | 9–14 agent-hours (single PR, 5 commit clusters) | `TestDDM_LegacyPath_BitForBit` GREEN at every commit; DDM reads `Restated()` + bridge ADDS DebtLikeClaims (opposite sign vs DCF/revenue_multiple which subtract — DDM derives equity FROM dividends and THEN EV from equity, so debt-like claims are added to derive EV); `grep -r '\.Raw()' internal/` returns no matches; translators/result-structs deleted; shadow snapshots byte-identical; `go test ./...` exit 0 |
| **Total** | | **~3 weeks + 1-2 days** | |
...
| Date | Change |
|------|--------|
| 2026-05-30 | Phase 5 PARTIAL on branch `dc1-phase-5` (tip `e6418e4`; 10 commits; NOT yet merged to master). Ships P5-C1 (DDM EV-bridge `+DebtLikeClaims` ADDED + CalcVersion 4.3→4.4) / P5-C2 (DDM consumer migration to `Restated()` via `LatestRestatedView` with 4-step bit-for-bit re-proof; JPM/BAC/WFC math preserved) / P5-C3 SCOPED (orchestrator firing-signal `nativeFired(...)` helper at `internal/services/datacleaner/firing_signal.go` filtering `LedgerEntry.Fired==true`) / P5-C5 PARTIAL (`cleaneddata.Raw()` deleted + concurrency contract godoc + `historicalData` slot KEEP). 2 review rounds: gpt-5.5 cross-model on initial 5-commit ship (zen-mcp continuation `22fbf842`) found 1 HIGH + 3 MEDIUM + 5 LOW, all closed in 5 fix commits (`83e6cb2`/`e4ea146`/`b12a870`/`de1a456`/`e6418e4`). Full `/execute` B-V-R-Q with VERIFIER/REVIEWER/QA subagents + gpt-5.5 Q-pass (zen-mcp continuation `bea446b5`) on the fixes returned VERIFIED/APPROVE_WITH_NITS/PASS/APPROVE. STILL DEFERRED to a follow-up session: (1) P5-C3-full Adjustments-projection (needs ARCH `Adjustment.Percentage` decision — path (a) preserve via `LedgerEntry.SkipMetrics["original_X"]` modifying ~5 Restater adjusters, OR path (b) accept `Percentage=0` lossy projection; closeout §6.1 recommends (a)); (2) P5-C4 translator + struct + dormant-fallback deletion (gated on (1)); (3) DDM `modelIBD` view-migration flip (bit-for-bit safe; deferred to minimize P5-C1's bit-for-bit surface); (4) DC-1 tracker archive + replay verification + fresh CalcVersion-4.4 baseline capture (gated on FULL DC-1 closure = P5-C4 done). Translator stack STILL load-bearing for `result.Adjustments` aggregation (orchestrator read still flows through per-rule translators; only firing-signal moved to native via P5-C3-scoped). Phase 5 closeout: `../archive/datacleaner-component-primitive-and-parallel-views-phase-5-closeout.md`. Next-session handoff: `../implementations/dc1-phase-5-next-session-handoff.md`. |
| 2026-05-27 | Phase 5 spec + implementer plan AUTHORED on branch `dc1-phase-5-prep` (worktree, forked from master `ce94f70` post-Phase-4). Spec: [datacleaner-component-primitive-and-parallel-views-phase-5-spec.md](../archive/datacleaner-component-primitive-and-parallel-views-phase-5-spec.md). Plan: [datacleaner-component-primitive-and-parallel-views-phase-5-implementation-plan.md](../archive/datacleaner-component-primitive-and-parallel-views-phase-5-implementation-plan.md). FINAL DC-1 phase. Scope: (1) DDM consumer migration to `Restated()` via `LatestRestatedView` with a 4-step bit-for-bit re-proof (key safety: DDM-consumed StockholdersEquity/NetIncome/DividendsPerShare are carried/identity-copied, NOT recomputed-umbrella, so Restated==latest when no Restater fires); (2) NEW DDM EV-bridge DebtLikeClaims correction — DDM analog of the Phase 4 `revenue_multiple` finding (ARCH recommends BUNDLE into Phase 5 as commit P5-C1, not a standalone Phase 4.x hotfix, since DDM's headline IntrinsicValue/EquityValue are dividend-derived and unaffected — only the derived EnterpriseValue drifts for B-rule banks); (3) legacy translator + adjustments-package result-struct + dead `entities.*` duplicate retirement via consumer-migration-then-deletion (P5-C3 → P5-C4); (4) dormant `earnings.go` legacy-fallback deletion; (5) `cleaneddata.Raw()` deletion; (6) accessor concurrency — formalize request-local contract, no `sync.Once` (recommended). `CalculationVersion 4.3 → 4.4` atomic with P5-C1; no `SchemaVersion` bump. Single PR, 5 commit clusters (P5-C1 EV-correction numeric drift isolated; P5-C2 load-bearing view migration isolated; P5-C3 behavior-preserving native aggregation; P5-C4 mechanical deletion; P5-C5 cleanup + closeout). Phase 5 → DC-1 done gate documented. Phase 5 row in "Phasing & implementation sequence" expanded to reference the new spec + plan. |
| 2026-05-27 | Phase 4 IMPLEMENTED on branch `dc1-phase-4` (5 commit clusters C-1..C-5: `f65604a`/`4f8a06c`/`9bc885e`/`7349a1e` + C-5 docs/closeout; awaiting REVIEWER + HUMAN merge). Consumer migration map realized: 13 valuation read sites moved to `cleaneddata` views; B3 routing flip (contingent → `InvestedCapital().DebtLikeClaims`, out of WACC denominator); dispatcher dual-writes deleted via §8.2.1 Option A (`applyLedgerComponentDeltas`); `CalculationVersion` 4.2→4.3. Consumer-migration-map view assignments largely as proposed, with two corrections recorded in the Phase 4 closeout: Graham + tangible value read `AsReported()` (the migration map's "tangible→Restated" assumed `Restated().TangibleAssets` is identity, but `Restated()` recomputes it from components → would drift). DDM migration, `Raw()` deletion, and the per-rule translator/`*AdjustmentResult` cleanup are the remaining Phase 5 items. Phase 4 closeout: `../archive/datacleaner-component-primitive-and-parallel-views-phase-4-closeout.md`. |
...
| 2026-05-23 | Phase 3 spec authored; covers view reconstruction (`CleanedFinancialData` with `AsReported`/`Restated`/`InvestedCapital` accessors), Q2 resolution (A2 `TaxShieldDTA = writedownAmount × EffectiveTaxRate`), Q4 resolution (B3 `AIProvenance.PromptHash` + `SourceDocHash` SHA-256 hex strings), `ctx context.Context` threading through `Process*Adjustments` signatures, and the translator-extraction decision (KEEP per-rule; Phase 4 deletes alongside dual-write). PR strategy: single PR recommended. Phase 3 ships zero downstream behavior change — consumer migration is Phase 4. Implementation plan filed at `datacleaner-component-primitive-and-parallel-views-phase-3-implementation-plan.md`. New spec lives at `datacleaner-component-primitive-and-parallel-views-phase-3-spec.md`. |
| 2026-05-26 | Phase 4 spec + implementer plan AUTHORED on branch `dc1-phase-4-prep` (worktree). Spec: [datacleaner-component-primitive-and-parallel-views-phase-4-spec.md](../archive/datacleaner-component-primitive-and-parallel-views-phase-4-spec.md). Plan: [datacleaner-component-primitive-and-parallel-views-phase-4-implementation-plan.md](../archive/datacleaner-component-primitive-and-parallel-views-phase-4-implementation-plan.md). Single-PR strategy with 5 commit clusters (C-1 plumbing → C-2 working-capital/ROIC + A2/A4/A5 dispatcher delete → C-3 DCF + cross-check + alt-model OI + C-rule dispatcher delete → C-4 WACC + EV→Equity bridge + B3 routing flip + A1/B-rule dispatcher delete + `CalculationVersion` 4.2 → 4.3 → C-5 Graham + tangible + DDM/currency confirmation + vestigial deletion + docs + closeout). 12 of 13 consumer sites migrate; DDM deferred to Phase 5 to preserve the JPM/BAC/WFC bit-for-bit invariant (added `TestDDM_ConsumerPath_UnaffectedByPhase4` as a Phase-4 guard pin). Dispatcher dual-write deletion uses Option A: dispatcher applies LedgerEntry component delta only (no umbrella mutation) — umbrellas recomputed in `Restated()`. New `dcf.CalculateEquityValueWithDebtLikeClaims` function added; legacy 5-arg `dcf.CalculateEquityValue` preserved for alt-model callers. No `SchemaVersion` bump (no new omitempty fields on `FinancialData` or `ValuationResult`). Phase 5 row in §"Phasing & implementation sequence" expanded to absorb DDM migration alongside `Raw()` cleanup. Replay drift expectation split into 4 classes; Class I tickers byte-identical except CalcVersion field, Class III B-rule firers show substantive WACC + EquityValue correction. Phase 4 → Phase 5 gate documented. Implementer plan estimates 12–18 agent-hours single PR. Estimated BACKEND dispatch after HUMAN spec review. |
| 2026-05-23 (PR-4) | Phase 2 PR-4 SHIPPED — Phase 2 COMPLETE: B1 operating_lease_capitalization (OverlayEmitter, `Field:"TotalDebt"`) `613773a`; B2 pension_underfunding (OverlayEmitter, `Field:"TotalDebt"`) `e328226`; B3 contingent_liability (OverlayEmitter, **`Field:"DebtLikeClaims"`** — Phase 4 routing intent; today's Phase 2 dual-write still mutates `data.TotalDebt`) with best-effort `AIProvenance` (`ModelName: b3AIModelName`; `PromptHash`/`SourceDocHash`/`ExtractedSpan` empty per Q4 deferral) `5c4e88d`. Orchestrator-level `data.TotalDebt += result.Amount` mutation absorbed into `ProcessLiabilityAdjustments`' per-rule switch arms (`37f7400`, Option α — keep wrapper, move mutation into wrapper). Liability-side shim branch deleted + both shim helpers (`shimLedgerEntriesFromLegacy` + `shimLedgerEntriesFromLegacyExcluding`) removed (`5eaadd3` — PR-1 shim FULLY gone). Basket integration test `TestLedger_BasketSnapshot_ClusterPrediction` added at `internal/integration/datacleaner_ledger_basket_test.go` (Task 4.6) — first integration test to read `data.AdjustmentLedger` directly and assert on per-ticker expected AdjusterID sets; 10/10 basket tickers PASS. Phase 2 closeout docs sweep (Task 4.7): CLAUDE.md per-PR sub-bullets replaced with consolidated Phase 2 SHIPPED bullet; spec/plan changelogs + DC-1 tracker progress + T2-BS-3 tracker (Option B carve-out) + THESIS/AGENTS/ARCHITECTURE/TESTING updates; NEW Phase 2 closeout doc at `docs/refactoring/archive/datacleaner-component-primitive-and-parallel-views-phase-2-closeout.md`. All 17 cleaner-side adjusters now native (6 A-rules + 7 C-rules + 3 B-rules + 1 PR-1 shim deletion across all 3 categories). Four role flavors canonized: OverlayEmitter, Restater, Restater+TaxShieldDTA, FlagEmitter. SchemaVersion stays at 8. T2-BS-3 disposition: Option B (carve-out) — parser fix deferred; AMD/KO `AsReported.TotalLiabilities=0` stays; Phase 3 `Restated` view reconstruction fixes downstream consumption. Phase 3 (CleanedFinancialData view reconstruction — `AsReported`/`Restated`/`InvestedCapital` accessors) is the next architectural gate; Phase 4 (13-site consumer migration + B3 routing flip + dual-write deletion) follows. Load-bearing invariants stayed GREEN through all 7 PR-4 commits: `TestDDM_LegacyPath_BitForBit` (non-trivial in PR-4 because B-rules touch `TotalDebt` — pinned by per-commit replay validation), `TestRecomputeUmbrellas_NoMutation`, `TestOrchestrator_LedgerOrdering`, `TestDataCleanerRecompute_ShadowMode_TickerBasket` + shadow-snapshot byte-identity, and the new `TestLedger_BasketSnapshot_ClusterPrediction`. Branch `dc1-phase-2-pr-4` final tip awaiting final REVIEWER pass + HUMAN merge of the full 4-PR stack to master. |
