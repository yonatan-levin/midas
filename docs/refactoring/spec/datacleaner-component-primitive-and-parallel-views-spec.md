...
| **5 — DDM migration + `Raw()` cleanup + vestigial deletion** | **DDM consumer migration** (deferred from Phase 4 per Phase 4 spec §7 to preserve the `TestDDM_LegacyPath_BitForBit` cross-Tier-2 contract) — flip `ddm.go:178/181/213/291/292` reads to `Restated()`. **Cleanup** — `cleaneddata.CleanedFinancialData.Raw()` (TODO(phase-5) marker present) becomes unreachable once DDM migrates; delete it. Delete any legacy `{Asset,Liability,Earnings}AdjustmentResult` structs deferred from Phase 4. Optionally stop populating `historicalData.Data[latestPeriod] = cleaningResult.CleanedData` if DDM was the last consumer. Optional: promote the three accessor methods to `sync.Once`-protected reads if a parallel-read consumer arrives. | 1-2 agent-hours (DDM) + 1 agent-hour (cleanup) | `TestDDM_LegacyPath_BitForBit` GREEN post-migration; `grep -r '\.Raw()' internal/` returns no matches; `go test ./...` exit 0 |
