...
**Version:** 0.1 DRAFT
**Date:** 2026-04-25
**Status:** PHASE 1 + 2.A + 2.B IMPLEMENTED — Phase 1 merged 2026-04-25 as `83cbfc7` (manual `?trace=1` / `X-Midas-Trace` triggers, plus the bundle-log-streams follow-up that closes QA-2026-04-25 MINOR-1 — `99-narrate.jsonl` + `99-debug-trace.jsonl` written via a `BundleSink` `zapcore.Core` wrapper). Phase 2.A merged 2026-04-27 as `48a9578` (auto-on-error trigger via `logging.artifact_store.triggers.on_error`; deferred-mode Bundle infrastructure). Phase 2.B merged 2026-04-29 as `fa89aa2` (auto-on-quality-flag trigger via `logging.artifact_store.triggers.quality_flag_threshold`; cleaner cache-hit path included; threshold normalisation + startup Warn; `trace.bundle.promoted` Info line). Phase 2.C (always-on), 2.D (replay tooling) explicitly deferred — see [Deferred Work](#13-deferred-work-phase-2).
