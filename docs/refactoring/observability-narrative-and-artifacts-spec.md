...
**Inherited follow-up (RESOLVED 2026-05-02, merge `621f805`):** BUG-013 (deferred-bundle `SetTicker` failed silently because `os.Rename` ran on a directory that didn't exist yet at handler-time; bundle promoted to `_no-ticker/` instead of `<TICKER>/`). Pre-existing — affected all three auto-triggers (on_error, on_quality_flag, always). Phase 2.C made it the dominant case because every request hit the path. Resolution: Option A from the BUG-013 spec — `SetTicker` branches on `b.deferred.Load()` and updates `b.root` in memory in deferred mode (skipping `os.Rename`); `Promote()` then `MkdirAll`s at the correct per-ticker path. Eager-mode behavior unchanged. Bundle now lands at `<date>/<TICKER>/req_<id>/` with manifest `outcome="ok"`. The fix also closed a pre-existing latent `b.root` race surface in `Promote` (snapshot under `b.mu` mirroring `runWorker` pattern) and added a TOCTOU re-check in `SetTicker` (under `pendingMu→b.mu`, against REVIEWER's reverse suggestion to honor `bufferStream`'s lock-ordering prohibition). Archived: `docs/bugs/archive/BUG-013-...md`.
...
| 2026-05-02 | v0.6 — §13.C inherited follow-up CLOSED. BUG-013 (deferred-bundle `SetTicker` no-op on `b.root`) merged as `621f805` on master (2 commits on `fix/bug013-deferred-bundle-set-ticker`: `f86d067` operator-UX fix + `021e362` REVIEWER follow-up for HIGH-A latent `b.root` race in `Promote` + MEDIUM-A TOCTOU between `b.deferred.Load()` and `b.mu.Lock()` in `SetTicker`). Option A from the BUG-013 spec applied: in deferred mode `SetTicker` skips `os.Rename` and updates `b.root` in memory; `Promote()` then `MkdirAll`s at the correct per-ticker path. Eager-mode behavior unchanged (existing `TestSetTicker_RenameFailureCountedAsWriteError` regression pin still passes). REVIEWER's HIGH-A and MEDIUM-A both addressed by snapshotting `b.root` under `b.mu` in `Promote` (mirrors `runWorker` pattern) and adding a re-check-after-lock in `SetTicker` under `pendingMu→b.mu` (chosen against REVIEWER's reverse-direction suggestion to honor `bufferStream`'s documented prohibition on `b.mu→pendingMu` nesting). QA performed independent lock-order audit (10 `b.mu` sites, 5 `pendingMu` sites; exactly two NEW `pendingMu→b.mu` nestings; ZERO `b.mu→pendingMu` sites anywhere). 150 race-detector iterations clean (`-race -count=3`). Live disk inspection: bundle landed at `artifacts/<date>/AAPL/req_<id>/` with manifest `outcome="ok"` and no `notes` field (the spurious `write_failures=1` from the pre-fix rename failure is gone). Coverage 90.3% (above 90% gate; 0.5pp dip from the deliberately non-deterministic race-test branches). 3 new race-detector tests added (`TestDeferredBundle_SetTickerPromoteRace_NoDataRace`, `TestSetTicker_DeferredBundle_AfterPromoteFallsThroughToEager`, `TestSetTicker_DeferredBundle_CloseDuringTOCTOURace`) + 4 BUG-013 unit tests + 1 eager regression pin + 3 integration test extensions (one per auto-trigger). BUG-013 doc archived to `docs/bugs/archive/`. Phase 2.D (replay tooling) remains the only deferred item. |
