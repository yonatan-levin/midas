...
5. **Observability narrative & artifacts — Phase 2.D** — Phase 2.A (auto-on-error) shipped 2026-04-27, Phase 2.B (auto-on-quality-flag) shipped 2026-04-29, Phase 2.C (always-on knob) shipped 2026-05-01. Only Phase 2.D remains: replay tooling (`cmd/replay/main.go` that re-runs a bundle through current code and diffs the output — whole separate feature, needs design first via `/plan-and-create`). Also pending: BUG-012 (runtime Warn on artifact-bundle buffer drops, inherited from Phase 2.A as B-4) and BUG-013 (deferred-bundle SetTicker doesn't update b.root, so all auto-triggered bundles land at `_no-ticker/` instead of `<TICKER>/` — affects all three auto-triggers; QA caught it during Phase 2.C QA where `Always=true` made it the dominant case). Tracked in §13 of `docs/refactoring/observability-narrative-and-artifacts-spec.md` and `docs/bugs/`; file individual `docs/reviewer/` items when prioritized.
