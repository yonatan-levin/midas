...
5. **Observability narrative & artifacts — Phase 2.C / 2.D** — Phase 2.A (auto-on-error) shipped 2026-04-27, Phase 2.B (auto-on-quality-flag) shipped 2026-04-29. Remaining: 2.C always-on knob (sustained debugging sessions — spec §13.C says "Trivial once 2.A/2.B exist; just an OR clause in the trigger evaluator"), 2.D replay tooling (`cmd/replay/main.go` that re-runs a bundle through current code and diffs the output — whole separate feature, needs design first via `/plan-and-create`). Also pending: BUG-012 (runtime Warn on artifact-bundle buffer drops, inherited from Phase 2.A as B-4). Tracked in §13 of `docs/refactoring/observability-narrative-and-artifacts-spec.md` and `docs/bugs/`; file individual `docs/reviewer/` items when prioritized.
