...
| Tier 2: AssumptionProfile (in flight) | Bootstrap + P0a + P0b + T2-P4-W1 SHIPPED (2026-05-16 / 2026-05-19) | `265b9c9` (Bootstrap) + `d2a586e` (P0a) + `2e48fde` (P0b) + `be92a79` (T2-P4-W1 merge) | Unified `AssumptionProfile` backbone keyed by `(archetype × maturity)` driving DCF/DDM/FFO/RevenueMultiple calibration. Closes RM-3 + VAL-1 + VAL-2 + VAL-3 P3. **Bootstrap** captured pre-Tier-2 baselines (10 bundles + DDM bit-for-bit goldens). **P0a** delivered the full profile package (91.5% coverage). **P0b** wires Tier 2 plumbing into the engine: `config/assumption_profiles.json` embeds; `Bundle.SetAssumptionProfileManifest` writes `08-assumption-profile.json` (schema v1); `service.go::performValuation` builds Facts + calls Resolve + stamps profile onto `ModelInput.Profile`/result/bundle; `NewService` 11th param `profile.Registry`; 11 omitempty fields added to `ModelResult`/`ValuationResult`/`FairValueResponse` (consumers no-op until P1-P4). JPM bit-for-bit DDM invariant intact across all 3 phases. **T2-P4-W1 SHIPPED 2026-05-19** (merge `be92a79`): classifier emission reconciled to `REIT_*` prefixed form (REIT_DATACENTER, REIT_INDUSTRIAL, REIT_RETAIL, …) so Tier 2 archetype rules in `config/assumption_profiles.json` will fire against real REIT subsector requests once P4 lands. Was discovered during P4 REVIEWER — pre-fix every REIT request would have fallen through to the `software_like_scaling:standard_growth` wildcard fallback after P4 merge, silently making the forward FFO path inert for the entire REIT category. Reconciled via config-driven approach (industry_codes.json + downstream consumers: router REIT-set, FFO subsector keys, sicToGICS map). **P1-P4 worktrees still pending rebase + merge onto fixed master** — RM-3 (forward revenue multiple), VAL-1 (DCF archetype-aware horizon + 5 diagnostics), VAL-2 (DDM multi-stage, bit-for-bit-load-bearing), VAL-3 P3 (forward FFO); each runs through its own B-V-R-Q gate cycle. Will bump engine to `CalculationVersion 4.2` at Tier 2 close. See `docs/refactoring/spec/assumption-profile-spec.md` + `docs/refactoring/implementations/assumption-profile-implementation-plan.md` + `docs/reviewer/T2-P4-W1-classifier-prefix-mismatch.md`. |
| DC-1: Datacleaner refactor (in flight) | Phase 0 SHIPPED 2026-05-16; Phase 1 SHIPPED 2026-05-19; **Phase 2 PR-1 + PR-2 + PR-3 SHIPPED 2026-05-22 (stacked on `dc1-phase-2-pr-3`, awaiting final merge after PR-4)**; PR-4 pending | `1640394` (P0 merge) / `2d916a7` (P1 merge) / `dc1-phase-2-pr-3` branch tip `4af3c33` (P2 PR-3 stacked) | Three-view datacleaner architecture — `CleanedFinancialData {AsReported, Restated, InvestedCapital}` with explicit `AdjustmentLedger` + `OverlaySpec` audit trail. Closes the post-clean balance-sheet asymmetry (`Assets ≠ Liabilities + Equity`) and unlocks future features (Altman-Z, P/B, ROE-decomp, distress screens). **Phase 0** added 4 plug fields to `FinancialData` (`OtherCurrentAssets`, `OtherNonCurrentAssets`, `OtherCurrentLiabilities`, `OtherNonCurrentLiabilities`) + `computePlugs` helper in `internal/infra/gateways/sec/plugs.go` populated at end of `parsePeriodData`. Zero behavior change — empirically replay-verified on AAPL + MSFT (timestamp-only drift in `17-response.json`). **Phase 1** added `internal/services/datacleaner/recompute.go::recomputeUmbrellas` — pure shadow-mode observer wired in at end of `CleanFinancialData` between `createRiskWarningFlags` and `calculateQualityScore`; emits WARN lines (`phase: "DC-1-P1-shadow"`) on every umbrella/component divergence without mutating `FinancialData`. Basket integration test records per-ticker JSON snapshots under `internal/integration/testdata/recompute-shadow/` as the Phase 2 punch-list input. **Phase 2 PR-1** introduced the unified `Adjuster` interface + `LedgerEntry`/`OverlaySpec`/`AdjustmentLedger`/`AmountSemantics`/`AIProvenance` entities + an intermediate dispatcher shim that maps legacy `entities.Adjustment` records into LedgerEntry shape (zero adjuster migration in PR-1; shim is the only ledger producer). **Phase 2 PR-2** migrates all 6 Category A adjusters (A1 goodwill, A2 intangible, A4 DTA, A5 inventory + TaxShieldDTA, plus 2 flag-only reviews) to implement `Adjuster` natively via a per-rule adapter pattern; introduces four observed role flavors (OverlayEmitter / Restater / Restater+TaxShieldDTA / FlagEmitter `Fired:false`-with-`Flags`); canonical mutation-FREE `Apply` with the dispatcher in `ProcessAssetAdjustments` owning the dual-write; deletes the asset-side shim branch. `CurrentSchemaVersions["FinancialData"]` bumped 7→8 atomic with Task 2.1. Bit-for-bit DDM (JPM/BAC/WFC) + recompute-shadow snapshot byte-identity preserved across all 7 PR-2 commits. A-FY-NULL investigation tracker filed at `docs/reviewer/DC-1-FY-enable-predicate-investigation.md` (HIGH-confidence NOT-A-BUG; heuristic fix punted to Phase 4+). **Phase 2 PR-3 SHIPPED 2026-05-22** (branch `dc1-phase-2-pr-3` tip `4af3c33`, stacked on PR-2): migrates all 7 Category C earnings adjusters (5 Restaters — C1 restructuring_charges, C2 asset_sale_gains, C3 litigation_settlements, C5 derivative_gains_losses with branch-divergent sign, C6 capitalized_interest with LOAD-BEARING `EquityOffset=0` special case for income-statement reclassification; plus 2 FlagEmitters — C4 stock_compensation and C7 working_capital_window_dressing) to the `Adjuster` interface natively; deletes the earnings-side shim branch in `service.go::applyActiveAdjustments` (mirrors PR-2's asset-side Task 2.6 deletion). C4 plan-vs-code disagreement resolved by trust-the-code precedent (plan said Restater; legacy code has no mutation → ships as FlagEmitter). SchemaVersion stays at 8 (no schema bump in PR-3). Shadow-snapshot byte-identity preserved (predicted-zero per plan §4 row C — earnings adjusters touch income-statement fields invisible to `recomputeUmbrellas` balance-sheet umbrella math). Bit-for-bit DDM + recompute-shadow byte-identity preserved across all 8 PR-3 commits. Liability-side shim branch + the two shim helpers preserved for PR-4. **PR-4 (B1/B2/B3 liability adjusters with B3 routing to `DebtLikeClaims`) pending**; then Phase 3 (view reconstruction) and Phase 4 (13-site consumer migration). Damodaran goodwill convention preserved (A1 Overlay excludes from `InvestedCapital`). See `docs/refactoring/spec/datacleaner-component-primitive-and-parallel-views-spec.md` + `docs/refactoring/implementations/datacleaner-component-primitive-and-parallel-views-phase-2-implementation-plan.md` + `docs/refactoring/implementations/datacleaner-component-primitive-and-parallel-views-phase-2-pr-3-handoff.md` + `docs/refactoring/implementations/datacleaner-component-primitive-and-parallel-views-phase-2-pr-4-handoff.md`. |
