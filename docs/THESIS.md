...
5. **Observability narrative & artifacts — Phase 2.B / 2.C / 2.D** — Phase 2.A (auto-on-error) shipped 2026-04-27. Remaining: 2.B auto-on-quality-flag (data-cleaner severity threshold), 2.C always-on knob (sustained debugging sessions), 2.D replay tooling (`cmd/replay/main.go` that re-runs a bundle through current code and diffs the output). 2.B and 2.C build on the deferred-mode `*Bundle` infrastructure 2.A introduced — each is roughly a new `case` arm in the trace-middleware switch plus a config key. Tracked in §13 of `docs/refactoring/observability-narrative-and-artifacts-spec.md`; file individual `docs/reviewer/` items when prioritized.
