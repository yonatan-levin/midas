...
| Tier 2: AssumptionProfile (in flight) | Bootstrap + P0a SHIPPED (2026-05-16) | `265b9c9` (Bootstrap) + `d2a586e` (P0a) | Unified `AssumptionProfile` backbone keyed by `(archetype × maturity)` driving DCF/DDM/FFO/RevenueMultiple calibration. Closes RM-3 + VAL-1 + VAL-2 + VAL-3 P3. **Bootstrap** captured pre-Tier-2 baselines (10 bundles + DDM bit-for-bit goldens). **P0a** delivered the full profile package: 21 Archetype constants, Facts DTO, Registry with SHA-256 config_hash, 9 validation invariants, pure 3-stage Resolve algorithm. 91.5% coverage. JPM bit-for-bit DDM invariant intact. **P0b dispatches next** (wires `Bundle.SetAssumptionProfileManifest`, populates `config/assumption_profiles.json`, integrates profile resolution into `service.go::performValuation` and `ModelInput`); then P1-P4 parallel worktrees. Will bump engine to `CalculationVersion 4.2` at Tier 2 close. See `docs/refactoring/spec/assumption-profile-spec.md` + `docs/refactoring/implementations/assumption-profile-implementation-plan.md`. |
