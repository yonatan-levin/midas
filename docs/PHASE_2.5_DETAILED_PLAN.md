...
- [ ] **Task 2.5.3: Contract Fuzzing (1 hour)**  
  **Sub-tasks**:  
    - [ ] Step 1: Generate OpenAPI spec from Gin routes (use swag if partial exists).  
    - [ ] Step 2: Install Schemathesis; run fuzz tests on /fair-value endpoint with invalid inputs (e.g., bad tickers).  
    - [ ] Step 3: Assert no 5xx errors; fix any via handler validation. Add TODO for CI integration and add these comments to TODO_TASKS_CATALOG.md for tracking.  
    - [ ] Step 4: Reality check—review fuzz logs; ensure RFC 7807 error format per rules.  
...
- [ ] **Task 2.5.4: Smoke CLI & Seed (1 hour)**  
  **Sub-tasks**:  
    - [ ] Step 1: Create scripts/smoke.sh: Curl /fair-value/AAPL with demo key from config.  
    - [ ] Step 2: Add SQL seed script (migrations/) for demo API key (non-hardcoded via ENV).  
    - [ ] Step 3: Update README.md with 30-second demo snippet, including output example.  
    - [ ] Step 4: Reality check—run smoke.sh; verify tangible/dcf values match expected (e.g., from testdata).  
...
- [ ] **Task 2.5.5: Perf Baseline (2 hours)**  
  **Sub-tasks**:  
    - [ ] Step 1: Install k6; write script for 20 RPS on /fair-value (use perplexity-ask for best practices).  
    - [ ] Step 2: Run for 60s; capture p95 latency/metrics (aim <300ms per rules).  
    - [ ] Step 3: Use pprof to inspect hotspots; optimize if needed (e.g., cache hit rate). Add comments/TODO for scaling and add these comments to TODO_TASKS_CATALOG.md for tracking.  
    - [ ] Step 4: Document baselines in docs/PERF_BASELINE.md; fix any linters.  
...
- [ ] **Task 2.5.6: MVP Freeze Checklist (1 hour)**  
  **Sub-tasks**:  
    - [ ] Step 1: Create docs/MVP_CHECKLIST.md: Items like "All tests pass", "Coverage ≥90%", "Perf SLA met".  
    - [ ] Step 2: Git tag v0.9.0-rc1; enforce bug-fix-only PRs.  
    - [ ] Step 3: Update NEW_INTEGRATION_PLAN.md with completion marks when updating NEW_INTEGRATION_PLAN.md only mark X next to the task that is completed if needed to add notes add them as footnotes at the end of the Phase 2.5 section.  
    - [ ] Step 4: Reality check—review checklist; only mark Phase 2.5 complete if all green.  
