...
- [x] Live MXL response confirmed — **superseded by the Phase-2 live-API verification below** (2026-06-29; see the Phase-2 "Live-API verification" row). The live applied value is **15.7×** (Damodaran "Semiconductor" EV/Sales), not the Phase-1 estimate of 6.5× — the warning now reads `Applied 15.7x EV/Revenue multiple for MFG_SEMI sector`. The unit test `TestRevenueMultipleModel_GetMultiple_RM2P1ContractFullSeven` still correctly pins 6.5× for the *Phase-1 `MFG_SEMI` bucket* (the fallback used only when no SIC maps to a Damodaran industry).
