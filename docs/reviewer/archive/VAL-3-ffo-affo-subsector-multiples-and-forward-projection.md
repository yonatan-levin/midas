...
**Status:** ALL PHASES SHIPPED (tracker closable) — filed 2026-05-06; Phase 1 + Phase 4 SHIPPED 2026-05-23 (subsector P/FFO multiples + subsector NAV cap rates via `getMultiple`/`getCapRate` longest-prefix-match in `internal/services/valuation/models/ffo.go`; DLR / SPG / PLD end-to-end subsector NAV tests in `ffo_test.go`). Phase 2 (AFFO) SHIPPED 2026-06-22 (#15); Phase 3 (forward FFO/AFFO projection with cost-of-equity discount) SHIPPED 2026-06-23 (#16) — forward leg now projects the AFFO-or-FFO headline base. VAL-3 P3 forward FFO scaffold (`Trailing/Forward/HorizonSelected/TerminalMultiple` fields, profile-gated horizon) shipped earlier in Tier 2 P4; Phase 3 here covers the broader integration with the unified profile work.
...
### Phase 3 (with unified profile work) — SHIPPED 2026-06-23 (#16)
- [x] Forward FFO/AFFO projection lands; profile-driven horizon. *(`ffo.go::Calculate` forward block projects `headlineBasePerShare` — AFFO/share when AFFO available, else FFO/share — over `Profile.HorizonYears`; gating unchanged: nil profile / horizon==0 leaves `ForwardValue=0` omitempty.)*
- [x] Cost-of-equity (not WACC) discount. *(forward leg discounts at `math.Pow(1+CostOfEquity, HorizonYears)`; pinned by `TestFFO_Forward_DiscountsAtCostOfEquity_NotWACC` — hand-computed value + WACC-invariance via `math.Float64bits`.)*
- [x] Both trailing and forward values emitted. *(`TestFFO_Forward_BothLegsEmitted_Divergence`; PLD 15–30%-higher and SPG ±10% pinned by `TestFFO_Forward_IndustrialREIT_PLD_ForwardHigherThanSnapshot` / `…_MallREIT_SPG_ForwardNearSnapshot` against explicit `Profile{}` per D1.)*
- [x] All RM-3 / VAL-1 / VAL-2 acceptance criteria holding (no cross-model regressions). *(`TestDDM_LegacyPath_BitForBit` + `TestFFOModel_Calculate_AFFO_AbsentBitForBit` green; recompute-shadow byte-identical; full suite minus the 4 artifact-gated baseline tests passes. C1 adds a `terminal_multiple` range guard in `profile/validation.go`. NO CalcVersion bump — stays 4.9, coordinated with Phase 2; NO `FairValueResponse`/`diff.go` change — forward/trailing stay on `ModelResult` (D3 deferred).)*
