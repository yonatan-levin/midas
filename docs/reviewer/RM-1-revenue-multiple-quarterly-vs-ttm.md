...
**Status: SHIPPED** — implementation merged to master via commits `2428ae1` (helper add), `3902703` (consumer wire-up), `9da6c68` (V/R/Q bridge-ordering follow-up), and `9a32d94` (RM-1.A T7 stale-data + RM-1.B doc sync). Merge point: `cfdf7b4`. Tracker status updated 2026-05-23 on branch `feat/rm-1-ttm-revenue-helper` to reflect the post-merge reality.

- [x] `entities.HistoricalFinancialData.TrailingTwelveMonthsRevenue()` lands with the documented fallback chain. Implementation at `internal/core/entities/financial_data.go:378-419`; the chain is `TTM_PRIOR_BRIDGE → TTM_4Q → ANNUAL_FY → ANNUALIZED_QUARTER → INSUFFICIENT_HISTORY` (bridge ordered first per the V/R/Q follow-up so the partial-year IPO shape surfaces as `TTM_PRIOR_BRIDGE` instead of being absorbed into `TTM_4Q`).
- [x] `revenue_multiple.go` consumes the helper instead of `latest.Revenue`. See `internal/services/valuation/models/revenue_multiple.go:96-98` — `revenue, source, ttmWarning := input.HistoricalData.TrailingTwelveMonthsRevenue()` with the source surfaced on `result.Warnings` for replay/dashboard inspection.
- [x] Tests T1–T9 above all pass with ≥90% coverage on the new helper. Test file: `internal/core/entities/trailing_revenue_test.go`. Per-function coverage on the 6 new helpers measured 2026-05-23: `TrailingTwelveMonthsRevenue` 100%, `ttmFourQuartersRevenue` 100%, `quartersAreContiguous` 100%, `ttmPriorBridgeRevenue` 94.7%, `latestAnnualRevenue` 94.4%, `annualizedQuarterRevenue` 92.3% (weighted aggregate well above the 90% finance floor).
- [ ] A live MXL response (filing_period=Q1) shows `dcf_value_per_share` of approximately $8.50 (TTM × current 1.5× multiple, before RM-2 fixes the multiplier). **Deferred to live-API verification pass.** Implementation is in place — replay-tooling validation via `cmd/replay --diff-stages` on the MXL bundle at `artifacts/2026-05-06/MXL/` would surface the warnings line `revenue_base: source=ANNUALIZED_QUARTER revenue=$548752000` and a per-share value of approximately $8.56 per the calculation in the Context table above; live API re-run is a separate Tier-2 closeout activity.
- [ ] A live AAPL response (filing_period=Q1) is unchanged or only mildly different from pre-fix. **Deferred to live-API verification pass.** Implementation is in place — for an AAPL-shape (FY + Q1) the helper picks `TTM_PRIOR_BRIDGE` and produces the same numeric answer that a 4-quarter sum would; live API re-run deferred alongside the MXL row above.
- [x] CHANGELOG/CLAUDE.md gotchas updated to note: "revenue_multiple model uses TTM revenue; the source is in the warnings string when the path was lossy." Added to project-root `CLAUDE.md` on this branch.
- [x] DC-1, RM-2, RM-3 cross-references updated to remove the "RM-1 latent bug" caveat. DC-1 and RM-2 carry no RM-1 caveat to remove (verified 2026-05-23). RM-3 still treats RM-1 as a precondition in its "Revenue base | $137M (Q1 only — see RM-1 bug)" line and `[ ] RM-1 (TTM revenue helper) has landed` checkbox — both flipped to the shipped state on this branch.
