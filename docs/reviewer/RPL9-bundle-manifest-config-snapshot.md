...
**Status:** RESOLVED — capture side SHIPPED 2026-05-23 on branch `feat/rpl-9-capture-side-config-snapshot`; **replay-side consumer SHIPPED 2026-06-29 on branch `feat/rpl9-replay-consumer` (GitHub #26).** The replay binary now reads the bundle's `00-config.json` and overlays it onto the hand-mirrored RPL-10 defaults; pre-1.2 bundles (no snapshot file) fall back to the hand-mirror unchanged, so RPL-10 remains the load-bearing fallback for old bundles (NOT deleted). The hand-copy config-drift bug class is now impossible by construction for 1.2+ bundles.
...
- [x] **Replay path reads the bundle's config (with fallback to current hand-copy for old bundles).** RESOLVED 2026-06-29: `replay/module.go::replayConfig(bundleDir)` now calls `artifact.ReadConfigSnapshot(bundleDir)` and overlays all 11 captured fields (9 `Valuation.*` + 2 `Macro.*`) onto the hand-mirrored base when a `00-config.json` is present; absent file (pre-1.2 bundle) → base returned unchanged; malformed file → error returned (fx construction aborts). The fx provider became `func() (*config.Config, error)`.
- [x] **Golden test: introduce a config divergence between replay/module.go and production viper defaults; verify replay STILL produces zero-diff because the bundle's saved config overrides.** RESOLVED 2026-06-29: `TestReplayConfig_BundleSnapshotOverridesHandMirror` (`internal/observability/replay/config_parity_test.go`) stages a `00-config.json` with all 11 fields deliberately divergent from the hand-mirror (e.g. `dcf_max_growth_rate` 0.42 vs default 0.50) and asserts `replayConfig` surfaces the BUNDLE values for every field — so a future field added to `ConfigSnapshot` without a matching overlay line fails here. Companion pins: `TestReplayConfig_AbsentBundleUsesHandMirror` (back-compat), `TestReplayConfig_MalformedSnapshotErrors` (fail-loud), `TestReadConfigSnapshot_{RoundTrip,AbsentFile,Malformed}` (reader).
...
- [x] **Inline comment in `replay/module.go` cycle-2 block reframed (the manifest now carries the source of truth for 1.2+ bundles).** RESOLVED 2026-06-29 — **reframed, not deleted** (the better choice per REVIEWER): the now-false claim that bundles "cannot be faithfully replayed until the manifest captures config" was removed, and the hand-mirror is recast as (a) the BASE before the overlay and (b) the sole fallback for pre-1.2 bundles. The RPL-10 lockstep-parity rationale stays because it remains load-bearing for old bundles. A `// keep in sync with artifact.ConfigSnapshot` cross-reference comment was added above the overlay block.
