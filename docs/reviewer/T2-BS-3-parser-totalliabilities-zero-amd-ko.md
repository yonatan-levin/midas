...
**Status:** RESOLVED (code) 2026-06-27 — Disposition: **Option A (parser fallback) APPLIED** on branch `fix/t2-bs-3-parser-totalliabilities` (GitHub issue #23), superseding the earlier Option B carve-out. The SEC parser now derives `TotalLiabilities = LiabilitiesCurrent + LiabilitiesNoncurrent` when the rolled-up `us-gaap:Liabilities` umbrella is absent or zero (`internal/infra/gateways/sec/parser.go::parsePeriodData`), so `AsReported.TotalLiabilities` becomes truthful for AMD/KO. Pinned by `TestParsePeriodData_TotalLiabilities_Fallback_T2BS3` (5 cases: umbrella-wins / umbrella-zero→fallback / both-splits→sum / current-only→0 / noncurrent-only→0). The former carve-out acceptance test `TestLedger_BasketSnapshot_T2BS3_RestatedReconstruction` was reworked → `TestLedger_BasketSnapshot_T2BS3_ParserTruthful` (asserts `AsReported.TotalLiabilities > 0`, skips when the tier2-baseline fixtures are absent per BUG-016). **One OPERATOR follow-up remains (NOT a code blocker):** the committed `internal/integration/testdata/recompute-shadow/{AMD,KO}.json` snapshots still pin the pre-fix `reported:0` divergences and must be regenerated against a captured `tier2-baseline` once the fixtures are restored (BUG-016) — the truthful `TotalLiabilities` changes Plug 4 (`OtherNonCurrentLiabilities`), so the shadow recompute will no longer diverge (`clamp_suspected` flips to `false`). GitHub issue: #23. Originally surfaced by DC-1 Phase 1 shadow-mode shim on first basket run (2026-05-19).
...
- DC-1 Phase 1 shadow-analysis report: `docs/refactoring/archive/datacleaner-component-primitive-and-parallel-views-phase-1-shadow-analysis.md` (cluster `B1-PARSER-TL-ZERO` describes this same finding)
...
| 2026-06-27 | **RESOLVED (code) — Option A APPLIED** (GitHub #23, branch `fix/t2-bs-3-parser-totalliabilities`). Added the `LiabilitiesCurrent + LiabilitiesNoncurrent` fallback in `parser.go::parsePeriodData` (umbrella present-and-nonzero still wins; both splits required, else stays 0; `NoncurrentLiabilities` IFRS alias included per REVIEWER L1). New unit test `TestParsePeriodData_TotalLiabilities_Fallback_T2BS3` (5 cases). Reworked the carve-out integration test → `TestLedger_BasketSnapshot_T2BS3_ParserTruthful` (Option A semantics + BUG-016 skip-when-absent). Gates: TDD red-green verified; full suite 47 pkgs green (3 pre-existing baseline failures unchanged); DDM bit-for-bit + gofmt + vet green; live AMD/KO HTTP 200 with no `derived total_liabilities` warnings and byte-stable precedence branch; REVIEWER verdict APPROVE_WITH_NITS (L1+N2 applied). **OPERATOR follow-up:** regenerate `recompute-shadow/{AMD,KO}.json` when the `tier2-baseline` fixtures are restored (BUG-016) — expected drift `reported:0 → reported:>0`, `clamp_suspected:true → false`. |
