...
### `Adjuster` interface (DC-1 Phase 2)

`internal/services/datacleaner/adjustments/adjuster.go` defines the `Adjuster` interface that every cleaner-side adjustment rule implements: `Apply(ctx, working, rule, cleaningCtx) (AdjusterOutput, error)` + `Name() string`. `AdjusterOutput` carries `LedgerEntries []entities.LedgerEntry`, `Overlays []entities.OverlaySpec`, and `Flags []entities.Flag`. Role flavors (Restater / OverlayEmitter / Hybrid / FlagEmitter) emerge from the **shape** of the returned output, not from a self-declared enum — keeping the orchestrator dispatch logic single-pass. The canonical Phase 2 pattern is **mutation-FREE Apply**: each `Apply` method reads `working` and returns AdjusterOutput; the dispatcher in `ProcessAssetAdjustments` (and, post-PR-3/PR-4, in the earnings/liability dispatchers) owns the dual-write `data.X = Y` / `data.X -= Z` mutations. Concentrating mutation at one call site per category means Phase 3's "delete the dual-write" reduces to a single deletion per category instead of N adjuster `Apply` methods. As of PR-2, all 6 Category A adjusters are native (A1 OverlayEmitter, A2/A4 Restaters, A5 Restater+TaxShieldDTA, 2 flag-only reviews using the FlagEmitter convention — Fired:false LedgerEntry with populated AdjusterOutput.Flags slice). As of PR-3, all 7 Category C adjusters are also native (5 Restaters — C1/C2/C3/C5/C6, where C6's `EquityOffset=0` is a LOAD-BEARING special case for capitalized-interest income-statement reclassification; plus 2 FlagEmitters — C4 ships as FlagEmitter despite the plan calling it Restater, per trust-the-code precedent, because legacy `ProcessStockCompensationAdjustment` does not mutate `data.NormalizedOperatingIncome`; and C7 working-capital). The earnings-side legacy shim branch in `service.go::applyActiveAdjustments` was deleted in PR-3 (Task 3.8 mirrors PR-2 Task 2.6). **As of PR-4, all 3 Category B liability adjusters are also native** (B1 OverlayEmitter `Field:"TotalDebt"`, B2 OverlayEmitter `Field:"TotalDebt"`, B3 OverlayEmitter `Field:"DebtLikeClaims"` with best-effort `AIProvenance`). Orchestrator-level `data.TotalDebt += result.Amount` mutation absorbed into the `ProcessLiabilityAdjustments` wrapper (Option α — keep wrapper, move mutation into wrapper). **PR-1 shim FULLY deleted** across all 3 categories (asset shim deleted PR-2 Task 2.6 `2c132aa`, earnings shim deleted PR-3 Task 3.8 `4af3c33`, liability shim + both helper functions `shimLedgerEntriesFromLegacy` + `shimLedgerEntriesFromLegacyExcluding` deleted PR-4 Task 4.5 `5eaadd3`). Phase 2 ships dual-write across the board (every `Apply*` is mutation-free; the dispatcher per-rule switch arm still performs the legacy `data.X += Y` mutation); **Phase 3** introduces `CleanedFinancialData` view reconstruction (`AsReported` / `Restated` / `InvestedCapital`) and **Phase 4** deletes the dual-write at the dispatcher's single call site per category (plus flips B3's consumer read site from `data.TotalDebt` to `Overlays[Field:"DebtLikeClaims"]`).
