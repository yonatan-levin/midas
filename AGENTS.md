...
| 17b | `docs/refactoring/spec/datacleaner-component-primitive-and-parallel-views-spec.md` + `docs/refactoring/implementations/datacleaner-component-primitive-and-parallel-views-phase-2-implementation-plan.md` + `docs/refactoring/implementations/datacleaner-component-primitive-and-parallel-views-phase-2-pr-3-handoff.md` | **DC-1 datacleaner refactor (in flight)** — moves the cleaner from in-place `FinancialData` mutation to a three-view output (`AsReported` / `Restated` / `InvestedCapital`) with an explicit `AdjustmentLedger` and `OverlaySpec` audit trail. Closes the balance-sheet asymmetry (today's cleaner mutates `TotalAssets` but not `StockholdersEquity` → `Assets ≠ Liabilities + Equity` post-clean) and unlocks future features (Altman-Z, P/B, ROE-decomposition, distress screens) that need a balanced post-clean balance sheet. **Phase 0 SHIPPED 2026-05-16** (merge `1640394`): 4 plug fields on `FinancialData` + `computePlugs` helper + wiring into `parsePeriodData`. Zero behavior change — populated but unread. **Phase 1 SHIPPED 2026-05-19** (merge `2d916a7`): `recomputeUmbrellas` shadow-mode observer + basket integration test producing committed per-ticker `recompute-shadow/<TICKER>.json` snapshots as the Phase 2 punch-list input. Zero behavior change. **Phase 2 PR-1 + PR-2 SHIPPED 2026-05-22** (stacked on branch `dc1-phase-2-pr-2`, tip `039f8ba`; awaiting final merge after PR-3/PR-4 land): PR-1 introduces the unified `Adjuster` interface with `AdjusterOutput{LedgerEntries, Overlays, Flags}` (one interface; Restater/Overlay/Hybrid roles emerge from output shape per GPT-5.5-pro pressure test) + LedgerEntry/OverlaySpec/AdjustmentLedger/AmountSemantics/AIProvenance entities + an intermediate orchestrator shim. PR-2 migrates all 6 Category A adjusters natively (A1 OverlayEmitter, A2/A4 Restaters, A5 Restater+TaxShieldDTA from `EffectiveTaxRate`, plus 2 flag-only reviews using a new "FlagEmitter" convention — Fired:false LedgerEntry with populated Flags slice signaling the review fired) via per-rule adapter types and a canonical mutation-FREE `Apply` (dispatcher owns dual-write); SchemaVersion["FinancialData"] bumped 7→8 atomic with Task 2.1; asset-side shim deleted (liability + earnings branches preserved for PR-3/PR-4). Bit-for-bit DDM + recompute-shadow byte-identity preserved across all 7 PR-2 commits. **PR-3 (earnings C1-C7) + PR-4 (liabilities B1/B2/B3 with B3 reroute to `DebtLikeClaims`, NOT TotalDebt — substantive accuracy correction) pending**; then Phase 3 view reconstruction → Phase 4 13-site consumer migration. Damodaran goodwill convention preserved (A1 stays Overlay, excluded from `InvestedCapital`). |
