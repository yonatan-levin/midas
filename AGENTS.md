...
| 17 | `docs/refactoring/observability-narrative-and-artifacts-spec.md` | Observability narrative + artifacts (Tier-1 narrate stream, Tier-2 Debug-tracer convention, Tier-3 per-request artifact bundle) — PHASE 1 + 2.A SHIPPED (manual `?trace=1`/`X-Midas-Trace` triggers + auto-on-error via `logging.artifact_store.triggers.on_error`); Phase 2.B (on_quality_flag), 2.C (always-on), 2.D (replay tooling) deferred — see spec §13 |
